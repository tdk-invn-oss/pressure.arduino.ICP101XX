...
#define LOG_ATMOSPHERICAL_PRESSURE 11.52608845149 // ln(101325)

#define P_WATER 9.80665

float convertToHeight(float pressure_p, float temperature_C)
{
    return PRESSURE_TO_HEIGHT_COEFF * TO_KELVIN(temperature_C) * (LOG_ATMOSPHERICAL_PRESSURE - log(pressure_p));
}

float convertToPressure(float height_m, float temperature_C)
{
    return ATMOSPHERICAL_PRESSURE_PA * exp(-HEIGHT_TO_PRESSURE_COEFF*height_m/TO_KELVIN(temperature_C));
}

float convertToDepth(float pressure_p)
{
    return (pressure_p - ATMOSPHERICAL_PRESSURE_PA)/P_WATER;
}